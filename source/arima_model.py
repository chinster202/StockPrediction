...
class StockARIMA:
    ...
    def check_stationarity(self, data, column="Close"):
        ...
        if is_stationary:
            ...
            print("⚠️  Series is NOT STATIONARY (p-value >= 0.05)")
            print("   Consider differencing or increasing d parameter")
    ...
    def fit(self, train_data, ticker_path, column="Close"):
        """
        ...
        try:
            ...
            self.model_fit.save(f"models/arima_model_{ticker_path}.pkl")
            ...
            print("   Try different order parameters")
            raise
    ...
    def predict_rolling(self, test_data, column="Close"):
        ...
        print(f"\n{'=' * 60}")
        print("MAKING ROLLING PREDICTIONS")
        print(f"{'=' * 60}")
...
def plot_predictions(train_data, test_data, predictions, column="Close"):
    ...
    ax2.plot(
        test_dates, test_data[column].values, label="Actual", marker="o", markersize=3
    )
...
def plot_residuals(test_data, predictions, column="Close"):
    ...
    percentage_within_1_percent = (
        np.mean(np.abs((predictions / test_data[column].values - 1) * 100) <= 1) * 100
    )
    ...
    print("✅ Prediction Error Percentage saved to results/arima_prediction_errors.png")
