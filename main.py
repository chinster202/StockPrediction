...
from source.model import StockLSTM, StockGRU
from source import train
from source.arima_model import StockARIMA, auto_arima_order, plot_predictions, plot_residuals
from source.trading_strategy import TradingSimulator, run_parameter_sweep 
import os
...
def main():
    ...
    for path in stock_files:
        ...
        yf_data = yf.download(path, start=config.start_date, end=config.end_date)

        print(f'datatype is {type(yf_data)} \n')

        # Save raw CSV with yfinance (includes metadata rows)
        yf_data.to_csv(f"data/{path}.csv")

        # Reload and remove the 2 metadata lines
        cleaned = pd.read_csv(f"data/{path}.csv", skiprows=2)
        ...
        cleaned.to_csv(f"data/{path}.csv")

        print(f"✅ Downloaded data for {path} from yfinance and saved to data/{path}.csv")

        # Get preprocessed data
        #stockdf, split_idx = stockdataloader.load_stock_data(f"data/{path}")
        # Use for local data directory

        print(f"\nLoading data for data/{path}.csv from yfinance...")

        stockdf, split_idx = stockdataloader.load_stock_data(f"data/{path}.csv")
        ...
        if config.model == "RNN":
            ...
            train_losses, val_losses, val_target, val_output = train.train_model(
                model, train_loader, val_loader, ticker_path = path, epochs=config.epochs, lr=config.lr
            )
            ...
            print("\nTraining complete! Best model saved as 'best_stock_model.bin'")
            ...
            # Fit model
            arima.fit(train_df, ticker_path = path, column=column)
    ...
    # Check if any sweep results were collected
    if any(v is not None for v in all_sweep_results.values()):
